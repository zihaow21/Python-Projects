...
class QueryClass(object):
    def __init__(self, query_type_data_dir):
        ...
        self.query_type_classcifier = fft.supervised(self.query_type_data_dir, 'query_type_model', label_prefix='__label__', thread=5, epoch=60)
    ...
    def queryTypeClass(self, query):
        label = self.query_type_classcifier.predict(query, 1)
...
query_type_dir ='/Users/ZW/Dropbox/data/query_type_test.txt'

query_class = QueryClass(query_type_dir)
result = query_class.queryTypeClass("where are you going on monday")[0]
print result
