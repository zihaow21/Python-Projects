...
class ChitChat:
    class TestMode:
        """
        structure representing the different testing modes
        """
        ALL = 'all'
    ...
    def __init__(self):
        ...
        # task object
        self.dataSerialization = None
        self.model = None

        # tensorflow utilities for saving/logging
        self.writer = None
        self.saver = None
        self.modelDir = ''  # where the model is saved
        self.globStep = 0  # represent the number of iteration
    ...
    def parseArgs(args):
        ...
        globalArgs.add_argument('--test', nargs='?', choices=[ChitChat.TestMode.ALL, ChitChat.TestMode.INTERACTIVE,
                                                              ChitChat.TestMode.DAEMON],
                                const=ChitChat.TestMode.ALL, default=None, help='if present, launch the program try to '
                                                                                'answer all sentences from data/test/ '
                                                                                'with the defined model(s), in '
                                                                                'interactive mode, the user can wrote '
                                                                                'his own sentences, use daemon mode to '
                                                                                'integrate the chatbot in another '
                                                                                'program')
        ...
        datasetArgs.add_argument('--corpus', choices=['cornell', 'opensubs'], default='cornell',
                                 help='corpus on which extract the dataset. Only two corpus available right now (cornell and opensubs)')
        ...
        datasetArgs.add_argument('--maxLength', type=int, default=20,
    ...
    def main(self, args=None):
        ...
        self.dataSerialization = DataSerialization(self.args)
        ...
        if self.args.test != ChitChat.TestMode.ALL:
        ...
        if self.args.test:
            if self.args.test == ChitChat.TestMode.INTERACTIVE:
                self.mainTestInteractive(self.sess)
            elif self.args.test == ChitChat.TestMode.ALL:
                print('Start predicting...')
                self.predictTestset(self.sess)
                print('All predictions done')
            elif self.args.test == ChitChat.TestMode.DAEMON:
        ...
        if self.args.test != ChitChat.TestMode.DAEMON:
            self.sess.close()
    ...
    def mainTrain(self, sess):
        ...
        try:  # If the user exit while training, we still try to save the model
            for e in range(self.args.numEpochs):
                ...
                print()
                print(
                "----- Epoch {}/{} ; (lr={}) -----".format(e + 1, self.args.numEpochs, self.args.learningRate))

                batches = self.dataSerialization.getBatches()
    ...
    def predictTestset(self, sess):
        ...
        if not modelList:
            print(
            'Warning: No model found in \'{}\'. Please train a model before trying to predict'.format(self.modelDir))
            return
        ...
        for modelName in sorted(modelList):
            ...
            with open(saveName, 'w') as f:
                ...
                for line in tqdm(lines, desc='Sentences'):
                    ...
                    predString = '{x[0]}{0}\n{x[1]}{1}\n\n'.format(question,
                                                                   self.dataSerialization.vec2sent(answer, clean=True),
    ...
    def mainTestInteractive(self, sess):
        ...
        while True:
            ...
            print('{}{}'.format(self.SENTENCES_PREFIX[1], self.dataSerialization.vec2sent(answer, clean=True)))

            if self.args.verbose:
                print(self.dataSerialization.batchVec2Sent(questionSeq, clean=True, reverse=True))
                print(self.dataSerialization.vec2sent(answer))
    ...
    def singlePredict(self, question, questionSeq=None):
        ...
        batch = self.dataSerialization.sent2enco(question)
        if not batch:
            return None
        ...
        answer = self.dataSerialization.deco2sentence(output)
    ...
    def daemonPredict(self, sentence):
        ...
        """
        return self.dataSerialization.vec2sent(
    ...
    def loadEmbedding(self, sess):
        ...
        with open(os.path.join(self.args.rootDir, 'data/word2vec/GoogleNews-vectors-negative300.bin'), "rb", 0) as f:
            ...
            initW = np.random.uniform(-0.25,0.25,(len(self.dataSerialization.word2id), vector_size))
            for line in tqdm(range(vocab_size)):
                ...
                while True:
                    ...
                    if ch != b'\n':
                        word.append(ch)
                if word in self.dataSerialization.word2id:
                    initW[self.dataSerialization.word2id[word]] = np.fromstring(f.read(binary_len), dtype='float32')
                else:
                    f.read(binary_len)
    ...
    def loadModelParams(self):
        ...
        if not self.args.reset and not self.args.createDataset and os.path.exists(configName):
            ...
            self.args.maxLength = config['General'].getint('maxLength')  # We need to restore the model length because of the textData associated and the vocabulary size
